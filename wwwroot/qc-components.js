...
class A extends HTMLElement {
  ...
  generateAnchors() {
    const e = [];
    return document.querySelectorAll(".tdm").forEach((t, i) => {
      const c = t.getAttribute("tdm-titre") || t.textContent.trim(), n = `tdm-anchor-${i}`;
      t.id = n, e.push(`<li><a href="#${n}" class="tdm-link">${c}</a></li>`);
    }), e.join("");
  }
  setupAnchors() {
    this.querySelectorAll(".tdm-link").forEach((s) => {
      s.addEventListener("click", (t) => {
        t.preventDefault();
        const i = s.getAttribute("href").substring(1), c = document.getElementById(i);
...
class k extends HTMLElement {
  ...
  get template() {
    const e = this.theme === "light" ? "theme-light" : "theme-dark", t = JSON.parse(this.medias || "[]").map((i) => `
            <li>
...
class q extends HTMLElement {
  ...
  closeAllAccordions() {
    document.querySelectorAll("qc-accordeon").forEach((s) => {
      const t = s.querySelector(".accordion-toggle"), i = t.nextElementSibling;
      t.setAttribute("aria-expanded", "false"), i.setAttribute("aria-hidden", "true"), i.style.display = "none";
      const c = t.querySelector(".lnr");
...
class L extends HTMLElement {
  ...
  addEventListeners() {
    this.querySelectorAll("button").forEach((s) => {
      s.addEventListener("click", (t) => {
  ...
  toggleSubmenu(e) {
    ...
    if (s && s.tagName.toLowerCase() === "ul") {
      const t = s.classList.toggle("show");
      e.setAttribute("aria-expanded", t), this.updateButtonIcon(e, t);
    }
  }
  updateButtonIcon(e, s) {
    const t = e.querySelector("li button span");
  ...
  enhanceAccessibility() {
    const e = this.shadowRoot.querySelector("nav"), s = this.querySelectorAll("ul");
